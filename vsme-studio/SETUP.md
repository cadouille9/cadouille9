...
1. **Merge to `main`.** The branch `claude/build-monetizable-product-qvsahd`
   contains the finished product. Open a PR, merge it.
2. **Enable GitHub Pages**: repo → *Settings → Pages → Source: GitHub Actions*.
   The included workflow deploys the `vsme-studio/` folder on every push to
   `main`. Your app is now at
   `https://cadouille9.github.io/cadouille9/` (landing) and `…/app.html`.
3. **Recommended — own domain** (~€10/year): buy e.g. `vsme-studio.de` or
...
4. *(Optional but tidy)* Move `vsme-studio/` into its own repository later —
   this repo is your GitHub profile repo. Nothing breaks either way; the
   deploy workflow only publishes the `vsme-studio/` folder.
